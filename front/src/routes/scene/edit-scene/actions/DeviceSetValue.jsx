...
class DeviceSetValue extends Component {
  ...
  onDeviceFeatureChange = (deviceFeature, device) => {
    ...
    // Display labels memorized for the canvas view, which renders the action without
    // resolving the selectors itself
    const featureName = deviceFeature.name || deviceFeature.selector;
    const label = device ? `${device.name} › ${featureName}` : featureName;
