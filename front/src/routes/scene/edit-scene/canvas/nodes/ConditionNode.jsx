    ...
    >
      ...
      {branching ? (
        <>
          ...
          <Handle type="source" position={Position.Bottom} id="then" style={{ left: '20%' }} />
          <span class={style.conditionHandleLabelThen}>Oui</span>

          {/* after (50%) — continuation du flux principal après la condition */}
          <Handle type="source" position={Position.Bottom} id="after" style={{ left: '50%' }} />
          <span class={style.conditionHandleLabelAfter}>Suite</span>

          {/* else (80%) — arête rouge dans graphToScene/SceneCanvas */}
          <Handle type="source" position={Position.Bottom} id="else" style={{ left: '80%' }} />
          <span class={style.conditionHandleLabelElse}>Non</span>
        </>
      ) : calendarCond ? (
        <>
          <Handle type="source" position={Position.Bottom} style={{ background: calendarHandleColor }} />
