...
class TurnOnLight extends Component {
  ...
  onDeviceFeaturesChange = (deviceFeatures, devices, isUserChange) => {
    ...
    // Display label memorized for the canvas view, which renders the trigger without
    // resolving the selectors itself
    this.props.updateTriggerProperty(
      this.props.index,
      ...
      deviceFeatures.length > 0
        ? deviceFeatures
            .map((feature, featureIndex) => {
              const featureName = feature.name || feature.selector;
              const device = devices[featureIndex];
              return device ? `${device.name} › ${featureName}` : featureName;
            })
            .join(', ')
        : null
    );
